 ...
 * - **Production**: off unless ``localStorage.setItem('munchkin_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('munchkin_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("munchkin_debug_ws")?.trim().toLowerCase() ?? "";
...
export class MunchkinClient {
  ...
  constructor(private options: MunchkinClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[munchkin ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[munchkin ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - MunchkinClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
