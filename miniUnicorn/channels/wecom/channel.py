...
# _sanitize_filename / 媒体扩展名集合由 miniUnicorn.channels._media_common 提供，
# 与 qq / dingtalk 共享同一份实现，避免历史跨频道重复定义。


def _guess_wecom_media_type(filename: str) -> str:
    """Classify file extension as WeCom media_type string.

    WeCom 的上传 API 用 ``"voice"`` 表示音频（不是 ``"audio"``），
    所以这里在共享 ``classify_media_type`` 的基础上做一次映射。
    """
