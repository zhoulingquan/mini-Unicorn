...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                self.logger.error("login failed. Run 'miniUnicorn channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if progress_event and (msg.metadata.get("_reasoning_delta") or msg.metadata.get("_reasoning")):
            self.logger.debug(
