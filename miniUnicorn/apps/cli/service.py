...
_CATALOG_SOURCES = (
    ...
    ("extensions", MINIUNICORN_EXTENSION_REGISTRY_URL, MINIUNICORN_EXTENSION_RAW_BASE, False),
)
...
class CliAppManager:
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from miniUnicorn."
        return f"""---
...
Use this skill when the user asks MiniUnicorn to operate {display} through its installed CLI app.
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_miniUnicorn_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy != "bundled":
            ...
            if still_managed or (not managed_entry_path and still_available):
                ...
                message = (
                    ...
                    "so MiniUnicorn kept it installed."
                )
                return self.payload() | {
                    "last_action": {
                        "ok": False,
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from miniUnicorn. {entry_point} "
                "is still available because it is managed outside MiniUnicorn."
            )
