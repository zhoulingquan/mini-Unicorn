...
from miniUnicorn.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/miniUnicorn",
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("MINIUNICORN_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
class OpenAICompatProvider(LLMProvider):
    ...
    async def chat_stream(
        ...
        await self._ensure_client()
        idle_timeout_s = int(os.environ.get("MINIUNICORN_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
