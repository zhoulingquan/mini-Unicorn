...
class CronTool(Tool, ContextAware):
    ...
    def __init__(self, cron_service: CronService, default_timezone: str = "system"):
        self._cron = cron_service
        self._default_timezone = default_timezone or "system"
    ...
    def _resolve_tz(tz_name: str):
        """Resolve timezone name to ZoneInfo, mapping 'system' to local timezone."""
        from zoneinfo import ZoneInfo
    ...
    def _display_timezone(self, schedule: CronSchedule) -> str:
        ...
        return schedule.tz or self._default_timezone or "system"

    @staticmethod
    def _format_timestamp(ms: int, tz_name: str) -> str:
        dt = datetime.fromtimestamp(ms / 1000, tz=CronTool._resolve_tz(tz_name or "system"))
        return f"{dt.isoformat()} ({tz_name or 'system'})"
    ...
    def _add_job(
        ...
        if every_seconds:
            ...
            effective_tz = tz or self._default_timezone or "system"
            ...
            if dt.tzinfo is None:
                default_tz = self._default_timezone or "system"
                if err := self._validate_timezone(default_tz):
                    return err
                dt = dt.replace(tzinfo=self._resolve_tz(default_tz))
