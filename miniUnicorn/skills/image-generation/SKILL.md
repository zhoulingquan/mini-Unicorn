...
If the `generate_image` tool is not available in the current tool list, tell the user that image generation is not enabled for this MiniUnicorn instance.
...
The tool stores generated images as persistent artifacts under MiniUnicorn's media directory and returns structured metadata:
...
```text
generate_image(
  prompt="A minimal app icon for MiniUnicorn: friendly robot head, rounded square, soft blue and white palette, clean vector style, no text",
  aspect_ratio="1:1",
  image_size="1K"
)
```
