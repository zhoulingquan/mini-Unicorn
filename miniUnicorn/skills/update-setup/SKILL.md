---
name: update-setup
description: One-time setup wizard for the MiniUnicorn upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `miniUnicorn --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v MiniUnicorn || true
python -m pip show miniUnicorn-ai || true
pipx list | sed -n '/miniUnicorn-ai/,+3p' || true
uv tool list | sed -n '/miniUnicorn-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how MiniUnicorn was
installed.
...
`question: "Where is your MiniUnicorn source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "miniUnicorn-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "miniUnicorn-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "miniUnicorn-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade MiniUnicorn to the latest version. Triggers: upgrade MiniUnicorn, update MiniUnicorn, 升级MiniUnicorn, 更新MiniUnicorn."
---
...
4. Use `exec` to verify: `miniUnicorn --version`
5. Tell the user the new version. Say: "Run `/restart` to restart MiniUnicorn and apply the update. If `/restart` is unavailable in this channel, restart the MiniUnicorn process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade MiniUnicorn' when you want to update."
