...
_TOOL_RESULTS_DIR = ".miniUnicorn/tool-results"
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 MiniUnicorn v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("miniUnicorn") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the MiniUnicorn package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("miniUnicorn") / "templates" / template_name
        if tpl.is_file():
