...
from munchkin.apps.protocol import app_manifest, compact_dict
from munchkin.config.paths import get_runtime_subdir
from munchkin.security.workspace_policy import is_path_within
...
_CATALOG_SOURCES = (
    ...
    ("extensions", MUNCHKIN_EXTENSION_REGISTRY_URL, MUNCHKIN_EXTENSION_RAW_BASE, False),
)
...
class CliAppManager:
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["munchkin_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from munchkin."
        return f"""---
...
Use this skill when the user asks Munchkin to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_munchkin_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- munchkin-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_munchkin_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from munchkin. {entry_point} "
                "is still available because it is managed outside Munchkin."
            )
