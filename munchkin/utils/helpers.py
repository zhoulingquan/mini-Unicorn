...
_TOOL_RESULTS_DIR = ".munchkin/tool-results"
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 Munchkin v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("munchkin") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the Munchkin package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("munchkin") / "templates" / template_name
        if tpl.is_file():
