...
from munchkin.agent.tools.context import current_request_session_key
from munchkin.agent.tools.exec_session import (
...
from munchkin.security.workspace_access import current_scope_allows_loopback, current_tool_workspace
...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (deny_patterns or []) + [
            ...
            r":\(\)\s*\{.*\};\s*:",          # fork bomb
            # Block writes to Munchkin internal state files (#2989).
    ...
    def _prepare_command(
        ...
        if self.path_append:
            if _IS_WINDOWS:
                ...
                env["MUNCHKIN_PATH_APPEND"] = self.path_append
                command = f'export PATH="$PATH{os.pathsep}$MUNCHKIN_PATH_APPEND"; {command}'
    ...
    def _guard_command(
        ...
        *,
        ...
        from munchkin.security.network import contains_internal_url
