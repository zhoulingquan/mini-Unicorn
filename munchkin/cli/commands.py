...
# Remove default handler and re-add with unified Munchkin format
logger.remove()
...
from munchkin import __logo__, __version__
...
from munchkin.config.paths import get_workspace_path, is_default_workspace
...
app = typer.Typer(
    ...
    help=f"{__logo__} Munchkin - Personal AI Assistant",
...
def _heartbeat_template() -> str | None:
    from munchkin.utils.helpers import load_bundled_template
...
def _init_prompt_session() -> None:
    ...
    from munchkin.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} Munchkin[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} Munchkin[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} Munchkin v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """Munchkin - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize Munchkin configuration and workspace."""
    from munchkin.config.loader import get_config_path, load_config, save_config, set_config_path
    from munchkin.config.schema import Config
    ...
    if wizard:
        ...
        try:
            ...
            console.print("[yellow]Please run 'munchkin onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'munchkin agent -m "Hello!"'
    gateway_cmd = "Munchkin gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} Munchkin is ready!")
    console.print("\nNext steps:")
    if wizard:
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from munchkin.channels.registry import discover_all
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from munchkin.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from munchkin.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from munchkin.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show Munchkin runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'munchkin-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from munchkin.api.server import create_app
    from munchkin.bus.queue import MessageBus
    from munchkin.session.manager import SessionManager

    if verbose:
        logger.enable("munchkin")
    else:
        logger.disable("munchkin")
...
def _load_or_create_desktop_config(config: str | None, workspace: str | None) -> Config:
    ...
    from munchkin.config.loader import (
        get_config_path,
        load_config,
    ...
    from munchkin.config.schema import Config as MunchkinConfig
...
def desktop_gateway(
    ...
    """Start the private local gateway used by Munchkin Desktop."""
...
def _run_gateway(
    ...
    *,
    ...
    from munchkin.channels.websocket import publish_runtime_model_update
    from munchkin.cron.service import CronService
    from munchkin.cron.types import CronJob
    from munchkin.providers.factory import build_provider_snapshot, load_provider_snapshot
    from munchkin.session.manager import SessionManager
    ...
    console.print(f"{__logo__} Starting Munchkin gateway version {__version__} on port {ws_port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show Munchkin runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from munchkin.channels.registry import discover_all
    from munchkin.config.loader import load_config, set_config_path
...
def channels_login(
    channel_name: str = typer.Argument(..., help="Channel name (use 'munchkin channels list' to see available)"),
...
def plugins_list():
    ...
    from munchkin.channels.registry import discover_all, discover_channel_names
...
def status():
    ...
    console.print(f"{__logo__} Munchkin Status\n")
