...
from munchkin.config.loader import load_config, resolve_config_env_vars, save_config
...
MCP_PRESETS: tuple[McpPreset, ...] = (
    ...
    McpPreset(
        ...
        category="search",
        description="Brave 搜索引擎，为 AI 提供网络搜索能力。",
        ...
        note="在 https://brave.com/search/api/ 免费申请 API Key。",
    ),
    McpPreset(
        name="memory",
        display_name="Memory",
...
def _custom_manifest(name: str, cfg: MCPServerConfig) -> dict[str, Any]:
    ...
    return app_manifest(
        app_id=name,
        display_name=name,
        description="Custom MCP server from Munchkin config.",
        category="custom",
        source="mcp-custom",
        brand_color="#64748B",
        capabilities=[
...
async def mcp_presets_test_action(query: QueryParams) -> dict[str, Any]:
    ...
    from munchkin.agent.tools.mcp import connect_mcp_servers
