---
name: update-setup
description: One-time setup wizard for the Munchkin upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `munchkin --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v Munchkin || true
python -m pip show munchkin-ai || true
pipx list | sed -n '/munchkin-ai/,+3p' || true
uv tool list | sed -n '/munchkin-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how Munchkin was
installed.
...
`question: "Where is your Munchkin source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "munchkin-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "munchkin-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "munchkin-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade Munchkin to the latest version. Triggers: upgrade Munchkin, update Munchkin, 升级Munchkin, 更新Munchkin."
---
...
4. Use `exec` to verify: `munchkin --version`
5. Tell the user the new version. Say: "Run `/restart` to restart Munchkin and apply the update. If `/restart` is unavailable in this channel, restart the Munchkin process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade Munchkin' when you want to update."
