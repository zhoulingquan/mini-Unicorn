...
from munchkin.command.builtin import cmd_new, register_builtin_commands
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("munchkin.agent.loop.SessionManager"), \
         patch("munchkin.agent.loop.SubagentManager") as MockSubMgr, \
         patch("munchkin.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates Munchkin onboard)."""
        from munchkin.config.loader import save_config
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from munchkin.agent.loop import UNIFIED_SESSION_KEY
        from munchkin.command.builtin import cmd_stop
