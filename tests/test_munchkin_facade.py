"""Tests for the Munchkin programmatic facade."""
...
from munchkin.munchkin import Munchkin, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Munchkin.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Munchkin.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("munchkin.config.loader.load_config") as mock_load, \
         patch("munchkin.providers.factory.make_provider") as mock_prov:
        mock_load.return_value = Config()
        mock_prov.return_value = MagicMock()
        mock_prov.return_value.get_default_model.return_value = "test"
        mock_prov.return_value.generation.max_tokens = 4096
        Munchkin.from_config()
...
async def test_run_hooks_restored_on_error(tmp_path):
    ...
    from munchkin.agent.hook import AgentHook
...
def test_workspace_override(tmp_path):
    ...
    bot = Munchkin.from_config(config_path, workspace=custom_ws)
...
def test_import_from_top_level():
    ...
    assert munchkin.Munchkin is Munchkin
    assert munchkin.RunResult is RunResult
