...
from miniUnicorn.apps.cli.service import CliAppError, CliAppManager, CliAppsRuntimeConfig
...
def test_payload_includes_miniUnicorn_extension_registry(tmp_path: Path) -> None:
    manager = _manager(tmp_path)
    _write_cache(manager._cache_path("harness"), {"meta": {"updated": "2026-04-16"}, "clis": []})
    ...
    assert app["manifest"]["trust"]["registry"] == "miniUnicorn-extension"
...
def test_install_records_entry_point_path_and_pip_distribution(
    ...
    monkeypatch.setattr(
        "miniUnicorn.apps.cli.service.importlib_metadata.distributions",
        lambda: [
            SimpleNamespace(
                entry_points=[
...
def test_fetch_skill_content_uses_extension_raw_base_for_relative_skills(
    ...
    assert seen == [
        "https://raw.githubusercontent.com/Re-bin/miniUnicorn-extension/main/skills/hyperframes/SKILL.md"
    ]
